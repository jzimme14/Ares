PULSONIX_LIBRARY_ASCII "SamacSys ECAD Model"
//15623131/1373120/2.50/4/4/Crystal or Oscillator

(asciiHeader
	(fileUnits MM)
)
(library Library_1
	(padStyleDef "r100_90"
		(holeDiam 0)
		(padShape (layerNumRef 1) (padShapeType Rect)  (shapeWidth 0.900) (shapeHeight 1.000))
		(padShape (layerNumRef 16) (padShapeType Ellipse)  (shapeWidth 0) (shapeHeight 0))
	)
	(textStyleDef "Normal"
		(font
			(fontType Stroke)
			(fontFace "Helvetica")
			(fontHeight 1.27)
			(strokeWidth 0.127)
		)
	)
	(patternDef "830107871909" (originalName "830107871909")
		(multiLayer
			(pad (padNum 1) (padStyleRef r100_90) (pt -0.825, -0.613) (rotation 90))
			(pad (padNum 2) (padStyleRef r100_90) (pt 0.825, -0.613) (rotation 90))
			(pad (padNum 3) (padStyleRef r100_90) (pt 0.825, 0.613) (rotation 90))
			(pad (padNum 4) (padStyleRef r100_90) (pt -0.825, 0.613) (rotation 90))
		)
		(layerContents (layerNumRef 18)
			(attr "RefDes" "RefDes" (pt 0.000, 0.000) (textStyleRef "Normal") (isVisible True))
		)
		(layerContents (layerNumRef 28)
			(line (pt -1.25 -1) (pt 1.25 -1) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt 1.25 -1) (pt 1.25 1) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt 1.25 1) (pt -1.25 1) (width 0.025))
		)
		(layerContents (layerNumRef 28)
			(line (pt -1.25 1) (pt -1.25 -1) (width 0.025))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt -2.325 2.063) (pt 2.325 2.063) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt 2.325 2.063) (pt 2.325 -2.063) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt 2.325 -2.063) (pt -2.325 -2.063) (width 0.1))
		)
		(layerContents (layerNumRef Courtyard_Top)
			(line (pt -2.325 -2.063) (pt -2.325 2.063) (width 0.1))
		)
		(layerContents (layerNumRef 18)
			(line (pt -0.9 -1.6) (pt -0.9 -1.6) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(arc (pt -0.9, -1.55) (radius 0.05) (startAngle 270) (sweepAngle 180.0) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt -0.9 -1.5) (pt -0.9 -1.5) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(arc (pt -0.9, -1.55) (radius 0.05) (startAngle 90.0) (sweepAngle 180.0) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(line (pt -0.9 -1.6) (pt -0.9 -1.6) (width 0.2))
		)
		(layerContents (layerNumRef 18)
			(arc (pt -0.9, -1.55) (radius 0.05) (startAngle 270) (sweepAngle 180.0) (width 0.2))
		)
	)
	(symbolDef "830107871909" (originalName "830107871909")

		(pin (pinNum 1) (pt 0 mils -100 mils) (rotation 0) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 230 mils -125 mils) (rotation 0]) (justify "Left") (textStyleRef "Normal"))
		))
		(pin (pinNum 2) (pt 1600 mils -100 mils) (rotation 180) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 1370 mils -125 mils) (rotation 0]) (justify "Right") (textStyleRef "Normal"))
		))
		(pin (pinNum 3) (pt 1600 mils 0 mils) (rotation 180) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 1370 mils -25 mils) (rotation 0]) (justify "Right") (textStyleRef "Normal"))
		))
		(pin (pinNum 4) (pt 0 mils 0 mils) (rotation 0) (pinLength 200 mils) (pinDisplay (dispPinName true)) (pinName (text (pt 230 mils -25 mils) (rotation 0]) (justify "Left") (textStyleRef "Normal"))
		))
		(line (pt 200 mils 100 mils) (pt 1400 mils 100 mils) (width 6 mils))
		(line (pt 1400 mils 100 mils) (pt 1400 mils -200 mils) (width 6 mils))
		(line (pt 1400 mils -200 mils) (pt 200 mils -200 mils) (width 6 mils))
		(line (pt 200 mils -200 mils) (pt 200 mils 100 mils) (width 6 mils))
		(attr "RefDes" "RefDes" (pt 1450 mils 300 mils) (justify Left) (isVisible True) (textStyleRef "Normal"))
		(attr "Type" "Type" (pt 1450 mils 200 mils) (justify Left) (isVisible True) (textStyleRef "Normal"))

	)
	(compDef "830107871909" (originalName "830107871909") (compHeader (numPins 4) (numParts 1) (refDesPrefix Y)
		)
		(compPin "1" (pinName "CRYSTAL_1") (partNum 1) (symPinNum 1) (gateEq 0) (pinEq 0) (pinType Unknown))
		(compPin "2" (pinName "GND_1") (partNum 1) (symPinNum 2) (gateEq 0) (pinEq 0) (pinType Unknown))
		(compPin "3" (pinName "CRYSTAL_2") (partNum 1) (symPinNum 3) (gateEq 0) (pinEq 0) (pinType Unknown))
		(compPin "4" (pinName "GND_2") (partNum 1) (symPinNum 4) (gateEq 0) (pinEq 0) (pinType Unknown))
		(attachedSymbol (partNum 1) (altType Normal) (symbolName "830107871909"))
		(attachedPattern (patternNum 1) (patternName "830107871909")
			(numPads 4)
			(padPinMap
				(padNum 1) (compPinRef "1")
				(padNum 2) (compPinRef "2")
				(padNum 3) (compPinRef "3")
				(padNum 4) (compPinRef "4")
			)
		)
		(attr "Mouser Part Number" "710-830107871909")
		(attr "Mouser Price/Stock" "https://www.mouser.co.uk/ProductDetail/Wurth-Elektronik/830107871909?qs=hWgE7mdIu5SqecU%2FcqYdCA%3D%3D")
		(attr "Manufacturer_Name" "Wurth Elektronik")
		(attr "Manufacturer_Part_Number" "830107871909")
		(attr "Description" "Crystals WE-XTAL 25.0MHz 10ppm 2.5 x 2.0mm")
		(attr "<Hyperlink>" "https://www.we-online.com/catalog/datasheet/830107871909.pdf")
		(attr "<Component Height>" "0.6")
		(attr "<STEP Filename>" "830107871909.stp")
		(attr "<STEP Offsets>" "X=0;Y=0;Z=0")
		(attr "<STEP Rotation>" "X=0;Y=0;Z=0")
	)

)
